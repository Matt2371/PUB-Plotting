...
The main record for data processing before we hand it over to the machine learning algorithm. 
...
```{r basin_data}
library(raster)
# CDEC Basin locations , replace with "cdec_fnf_stations_data.csv" when you have SFJ and OTR data
sptdf <- df <- read.csv("D:/ml with cdec uf/Input Data/CDEC_FNF/cdec_fnf_stations_data_minus_sfj_otr.csv", header=TRUE, stringsAsFactors=FALSE, fileEncoding="latin1")
...
# read in
basins_inc@data$TMP <- read.csv("D:/ml with cdec uf/Input Data/CDEC_FNF/basins_inc_PRISM_TMP.csv")
basins_inc@data$PPT <- read.csv("D:/ml with cdec uf/Input Data/CDEC_FNF/basins_inc_PRISM_PPT.csv")

# fix the column names as actual dates
strspl <- unlist(strsplit(colnames(basins_inc@data$TMP), split="_"))
...
colnames(basins_inc@data$TMP) <- colnames(basins_inc@data$PPT) <- date_formatted
...
basins_inc@data$TMPLAG1 <- tmplag1[1:(length(tmplag1)-1)]
basins_inc@data$TMPLAG2 <- tmplag2[1:(length(tmplag1)-2)]
basins_inc@data$TMPLAG3 <- tmplag3[1:(length(tmplag1)-3)]
...
basins_inc@data$PPTLAG1 <- pptlag1[1:(length(pptlag1)-1)]
basins_inc@data$PPTLAG2 <- pptlag2[1:(length(pptlag1)-2)]
basins_inc@data$PPTLAG3 <- pptlag3[1:(length(pptlag1)-3)]
```
...
snow_df_long <- reshape2:::melt.data.frame(y, id.var="DATE", measure.var = 1:(ncol(y)-1), variable.name = "CDEC_ID", value.name = "SNOW")
```
...
basins_inc_compactness <- basins_inc@data$AREASQKM*1000000/(basins_inc_perimeter^2)
basins_inc@data$COMPACTNESS <- basins_inc_compactness
...
basins_inc@data$MEANELEV <- test$V1
...
basins_inc@data$MAXELEV <- test$V1

# basin relief ratio: the ratio between total relief (max elev-min elev) and basin length (long axis length)
basins_inc@data$BASINRELIEFRATIO <- (basins_inc@data$MAXELEV-sptdf@data$GAUGEELEV[c(17,20,21,22)])/basins_inc@data$LENGTH
...
cdec_fnf_sta <- read.csv("D:/ml with cdec uf/Input Data/CDEC_FNF/cdec_fnf_stations_data_minus_sfj_otr.csv", header=TRUE, stringsAsFactors=FALSE, fileEncoding="latin1")
id_list_cdec <- cdec_fnf_sta$CDEC_ID

# # you can remove c("SFR", "FTM", "BHN", "SJM") because their records sometimes do not overlap with the others, they are stations that CDEC retired
...
CDECquery <- function(id, sensor, interval='D', start, end) {
  ...
  c <- data.frame(cbind.data.frame(DATE=d$datetime, CDEC_ID=d$STATION_ID, FLOW=as.numeric(d$VALUE)))
...
for (id in id_list_cdec){
  ...
  # had to aggregate because there seems to be non-unique values for the same date in the dataset
...
library(reshape2)
tbd <- melt(cdec_fnf_long, id.vars=c("DATE", "CDEC_ID"))
cdec_fnf_wide <- dcast(tbd, DATE~CDEC_ID)
...
write.csv(cdec_fnf_wide_inc, "D:/ml with cdec uf/Intermediary Data/cdec_fnf_wide.csv")
...
```{r month}
#fix the date
cdec_fnf$DATE <- as.Date(cdec_fnf$DATE, "%m/%d/%Y")
...
## 2.0 Data Prep for modelling  
```{r bind_data}
# add the "###_INC" basins to the df dataframe
df <- rbind(df, df)
df$CDEC_ID <- basins_inc@data$STATION

df$AREASQM <- basins_inc@data$AREASQKM[c(1,2,4)]
df$SHAPE <- basins_inc@data$SHAPE[c(1,2,4)]
df$COMPACTNESS <- basins_inc@data$COMPACTNESS[c(1,2,4)]
df$MEANELEV <- basins_inc@data$MEANELEV[c(1,2,4)]
df$BASINRELIEFRATIO <- basins_inc@data$BASINRELIEFRATIO[c(1,2,4)]
df$KSAT <- basins_inc@data$KSAT[c(1,2,4)]
df$SILT <- basins_inc@data$SILT[c(1,2,4)]
df$SAND <- basins_inc@data$SAND[c(1,2,4)]
df$CLAY <- basins_inc@data$CLAY[c(1,2,4)]
df$AWC <- basins_inc@data$AWC[c(1,2,4)]
df$LAMBDA <- basins_inc@data$LAMBDA[c(1,2,4)]
df$N <- basins_inc@data$N[c(1,2,4)]
df$RESDT <- basins_inc@data$RESDT[c(1,2,4)]
df$VEGETATED <- basins_inc@data$VEGETATED[c(1,2,4)]
df$DOMGEOLOGY <- basins_inc@data$DOMGEOLOGY[c(1,2,4)]

cdec_fnf_merge <- merge(cdec_fnf, df, by="CDEC_ID")

moddf <- merge(cdec_fnf_merge, tmp_df_long, by=c("DATE","CDEC_ID"))
...
colnames(moddf)
#  [1] "DATE"         "CDEC_ID"          "MONTH"            "FLOW"             "MONTH_ORDINAL"   
#  [6] "SEASON"           "YEAR"             "LATITUDE"         "LONGITUDE"        "STATION_NAME"    
# [11] "RIVER_BASIN"      "COUNTY"           "ELEVATION_FT"     "OPERATOR"         "STATIONS_ABOVE1" 
# [16] "STATIONS_ABOVE2"  "STATIONS_ABOVE3"  "STATIONS_ABOVE4"  "AREASQM"          "SHAPE"           
# [21] "COMPACTNESS"      "MEANELEV"         "BASINRELIEFRATIO" "KSAT"             "SILT"            
# [26] "SAND"             "CLAY"             "AWC"              "LAMBDA"           "N"               
# [31] "RESDT"            "VEGETATED"        "DOMGEOLOGY"       "tmp"              "tmplag1"         
# [36] "tmplag2"          "tmplag3"          "ppt"              "pptlag1"          "pptlag2"         
# [41] "pptlag3"          "SNOW"

# order the columns somewhat logically
moddf <- moddf[, c("DATE", "CDEC_ID", "STATION_NAME", "RIVER_BASIN", "COUNTY", "OPERATOR", "STATIONS_ABOVE1", "STATIONS_ABOVE2" , "STATIONS_ABOVE3", "STATIONS_ABOVE4", "LONGITUDE", "LATITUDE", "MONTH", "MONTH_ORDINAL", "SEASON", "YEAR", "TMP", "TMPLAG1", "TMPLAG2", "TMPLAG3", "PPT", "PPTLAG1", "PPTLAG2", "PPTLAG3", "SNOW", "AREASQM", "SHAPE", "COMPACTNESS", "MEANELEV", "BASINRELIEFRATIO", "KSAT", "SILT", "SAND", "CLAY", "AWC", "LAMBDA", "N", "RESDT", "VEGETATED", "DOMGEOLOGY", "FLOW")]
...
str(moddf)
moddf$MONTH <- as.factor(moddf$MONTH)
str(moddf)

# output the dataframe to a .rds. use this for larger data 
saveRDS(moddf, file="D:/ml with cdec uf/Intermediary Data/moddf_inc.rds" )

# now just read it back in
moddf <- readRDS("D:/ml with cdec uf/Intermediary Data/moddf_inc.rds")
